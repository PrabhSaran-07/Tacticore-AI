      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'center' }}>
            <SvgIcon name="loading" className="animate-spin" size="4rem" color="var(--primary)" />
          </div>
        ...
        <div className="card" style={{ maxWidth: '500px', width: '100%', textAlign: 'center', padding: '2rem' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--gray-400)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem' }}>
            {session.title && <><strong>{session.title}</strong> · </>}
            <SvgIcon name="⏱" size="0.95rem" /> {session.timeLimit} min
          </p>
        </div>
    ...
    return (
      <div style={{
        ...
        <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'center' }}>
          <SvgIcon name="🏁" size="5rem" color="var(--primary)" />
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--gray-100)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <button onClick={() => copyCode(session.sessionCode)} style={{ background: 'none', border: 'none', color: 'var(--gray-400)', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center' }} title="Copy Session Code"><SvgIcon name="📋" size="1.25rem" /></button>
            </h1>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <button className="btn btn-success" onClick={handleStartSession} disabled={participants.length === 0}
              style={{ fontSize: '1.1rem', padding: '0.75rem 2rem', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}>
              <SvgIcon name="▶" /> Start Session ({participants.length} cadets)
            </button>
        ...
        <div className="card">
          ...
          {participants.length === 0 ? (
            <div style={{ color: 'var(--gray-500)', fontStyle: 'italic', textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <button className="btn btn-sm btn-secondary" onClick={() => copyCode(session.sessionCode)} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.2rem' }}><SvgIcon name="📋" size="0.85rem" /> Copy Code</button>
            </div>
          ) : (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', position: 'relative', flex: 1, minHeight: 0, overflow: 'hidden' }}>
      ...
      {showBriefing && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="card" style={{ maxWidth: '650px', width: '100%', boxShadow: '0 0 40px rgba(59,130,246,0.3)' }}>
            <h2 className="card-title" style={{ marginBottom: '1rem', color: 'var(--primary)', fontSize: '1.75rem', display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="📋" /> Situation Briefing</h2>
            <div style={{ color: 'var(--gray-300)', lineHeight: '1.6', fontSize: '0.95rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div><strong>Resources:</strong><ul style={{ listStyle: 'none', padding: 0, marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  {session.assignedResources?.fireTrucks > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="🚒" size="1rem" /> {session.assignedResources.fireTrucks} Fire Trucks</li>}
                  {session.assignedResources?.volunteers > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="👷" size="1rem" /> {session.assignedResources.volunteers} Volunteers</li>}
                  {session.assignedResources?.waterPumps > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="💧" size="1rem" /> {session.assignedResources.waterPumps} Water Pumps</li>}
                  {session.assignedResources?.ambulance > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="🚑" size="1rem" /> {session.assignedResources.ambulance} Ambulance</li>}
                  {session.assignedResources?.police > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="🚓" size="1rem" /> {session.assignedResources.police} Police</li>}
                  {session.assignedResources?.citizen > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="🚶" size="1rem" /> {session.assignedResources.citizen} Citizens</li>}
                  {session.assignedResources?.car > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="🚗" size="1rem" /> {session.assignedResources.car} Cars</li>}
                  {session.assignedResources?.bike > 0 && <li style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="🚲" size="1rem" /> {session.assignedResources.bike} Bikes</li>}
                  {(session.assignedResources?.customItems || []).map((ci, idx) => (
                    <li key={idx} style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="📦" size="1rem" /> {ci.quantity} {ci.name}</li>
                  ))}
                </ul></div>
                <div><strong>Info:</strong><ul style={{ listStyle: 'none', padding: 0, marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}><li style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}><SvgIcon name="⏱" size="0.9rem" /> {session.timeLimit} min</li><li style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}><SvgIcon name="🎖" size="0.9rem" /> Chest No: {user?.chestNo || 'N/A'}</li></ul></div>
              </div>
            </div>
            <button className="btn btn-primary" style={{ width: '100%', padding: '1rem', marginTop: '1rem', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem' }} onClick={() => setShowBriefing(false)}><SvgIcon name="🚀" /> Begin Planning</button>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="card" style={{ maxWidth: '500px', width: '100%' }}>
            {submitStatus === 'success' ? (
              <div style={{ textAlign: 'center', padding: '2rem 0' }}><div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'center' }}><SvgIcon name="✅" size="4rem" color="var(--success)" /></div><h2 style={{ color: 'var(--success)' }}>Submitted!</h2><p style={{ color: 'var(--gray-400)' }}>Redirecting...</p></div>
            ) : (<>
              <h2 className="card-title" style={{ marginBottom: '1rem', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="📤" /> Submit Answer</h2>
              ...
              {submitStatus === 'error' && <p style={{ color: 'var(--danger)', fontSize: '0.85rem', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.2rem' }}><SvgIcon name="⚠" size="0.95rem" /> Failed. Try again.</p>}
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <button className="btn btn-primary" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem' }} onClick={() => handleSubmitAnswer('manual')} disabled={submitStatus === 'submitting'}>{submitStatus === 'submitting' ? <><SvgIcon name="loading" className="animate-spin" /> ...</> : <><SvgIcon name="📤" /> Confirm</>}</button>
      ...
      {!isRunning && !showBriefing && !showSubmitConfirm && session?.phase !== 'waiting' && session?.phase !== 'completed' && isCadet && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card" style={{ maxWidth: '500px', width: '100%', textAlign: 'center' }}>
            <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'center' }}><SvgIcon name="⏸" size="4rem" color="var(--warning)" /></div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div className="card" style={{ maxWidth: '600px', width: '100%', maxHeight: '85vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h2 className="card-title" style={{ color: 'var(--primary)', fontSize: '1.5rem', margin: 0, display: 'flex', alignItems: 'center', gap: '0.35rem' }}><SvgIcon name="📖" /> Instructions</h2>
              <button onClick={() => setShowInstructions(false)} style={{ background: 'none', border: 'none', color: 'var(--gray-400)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}><SvgIcon name="✕" size="1.25rem" /></button>
            </div>
            <div style={{ color: 'var(--gray-300)', fontSize: '0.85rem', lineHeight: '1.7' }}>
              {[
                { title: 'Map', icon: '🗺', color: 'var(--primary)', items: ['View Mode: drag to pan, scroll to zoom', 'Select resource then click map to place', 'Draw Route: click points → Finish Route', 'Undo removes last item, Clear removes all'] },
                { title: 'Chat & Submit', icon: '💬', color: 'var(--success)', items: ['Discuss plan in real-time', 'Click Submit when ready, auto-submit on timeout'] },
              ].map((s, i) => (
                <div key={i} style={{ marginBottom: '0.75rem', padding: '0.6rem', background: 'var(--gray-800)', borderRadius: '0.4rem' }}>
                  <strong style={{ color: s.color, display: 'flex', alignItems: 'center', gap: '0.25rem' }}><SvgIcon name={s.icon} size="0.95rem" /> {s.title}</strong>
      ...
      {isCadet && (<>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.35rem 0.75rem', background: 'var(--gray-900)', borderBottom: '1px solid rgba(14,165,233,0.2)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            ...
            <button className="btn btn-sm btn-success" onClick={() => setShowSubmitConfirm(true)} style={{ padding: '0.3rem 0.7rem', display: 'inline-flex', alignItems: 'center', gap: '0.2rem' }}><SvgIcon name="📤" /> Submit</button>
            <button className="btn btn-sm btn-secondary" onClick={() => setShowBriefing(true)} style={{ padding: '0.3rem 0.5rem', display: 'inline-flex', alignItems: 'center' }}><SvgIcon name="📋" size="0.9rem" /></button>
            <button className="btn btn-sm btn-secondary" onClick={() => setShowInstructions(true)} style={{ padding: '0.3rem 0.5rem', display: 'inline-flex', alignItems: 'center' }}><SvgIcon name="📖" size="0.9rem" /></button>
            <button className="btn btn-sm btn-secondary" onClick={() => { if (onLogout) onLogout(); }} style={{ padding: '0.3rem 0.5rem', display: 'inline-flex', alignItems: 'center' }}><SvgIcon name="⏏" size="0.9rem" /></button>
          </div>
        </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.2rem' }}>
                  {[
                    ...
                    ...(session?.assignedResources?.customItems || []).map(ci => ({
                      k: `add_custom_${ci.name}`, i: ci.name, l: ci.name.substring(0,8), mx: ci.quantity
                    }))
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card" style={{ maxWidth: '500px', width: '100%', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
              <button className="btn btn-primary" onClick={() => { setShowTimeUp(false); navigate(`/accessor`); }} style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}><SvgIcon name="📥" size="0.95rem" /> Check Submissions</button>
              <button className="btn btn-danger" onClick={() => { setShowTimeUp(false); handleEndSession(); }} style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}><SvgIcon name="🏁" size="0.95rem" /> End Session</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAllSubmitted && isAccessor && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card" style={{ maxWidth: '500px', width: '100%', textAlign: 'center' }}>
            <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'center' }}><SvgIcon name="✅" size="4rem" color="var(--success)" /></div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
              ...
              <button className="btn btn-danger" onClick={() => { setShowAllSubmitted(false); handleEndSession(); }} style={{ display: 'flex', alignItems: 'center', gap: '0.2rem' }}><SvgIcon name="🏁" size="0.95rem" /> End Session</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isAccessor && (<>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.3rem 0.75rem', background: 'var(--gray-900)', borderBottom: '1px solid rgba(14,165,233,0.2)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
            ...
            <button onClick={toggleSimulation} className={`btn btn-sm ${isRunning ? 'btn-danger' : 'btn-success'}`} style={{ padding: '0.25rem 0.6rem', display: 'inline-flex', alignItems: 'center' }}>{isRunning ? <SvgIcon name="⏸" size="0.85rem" /> : <SvgIcon name="▶" size="0.85rem" />}</button>
            <button className="btn btn-sm btn-danger" onClick={handleEndSession} style={{ padding: '0.25rem 0.6rem', display: 'inline-flex', alignItems: 'center', gap: '0.15rem' }}><SvgIcon name="🏁" size="0.85rem" /> End</button>
            <button className="btn btn-sm btn-secondary" onClick={() => setShowBriefing(true)} style={{ padding: '0.25rem 0.4rem', display: 'inline-flex', alignItems: 'center' }}><SvgIcon name="📋" size="0.85rem" /></button>
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '75% 25%', flex: 1, minHeight: 0, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '0.5rem 0.6rem', borderBottom: '1px solid rgba(14,165,233,0.08)' }}>
                ...
                {[
                  ...
                  { k: 'add_bike', i: '🚲', l: 'Bikes', mx: session?.assignedResources?.bike || 0 },
                  ...(session?.assignedResources?.customItems || []).map(ci => ({
                    k: `add_custom_${ci.name}`, i: ci.name, l: ci.name, mx: ci.quantity
                  }))
                ].filter(r => r.mx > 0).map(r => {
                  const used = currentMarkers.filter(m => m.type === r.k).length;
